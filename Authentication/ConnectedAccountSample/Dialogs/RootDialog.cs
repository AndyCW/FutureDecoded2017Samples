...
namespace ConnectedAccountSample.Dialogs
{
    ...
    public class RootDialog : IDialog<object>
    {
        ...
        private async Task MessageReceivedAsync(IDialogContext context, IAwaitable<object> result)
        {
            ...
            {
                if (activity.Text.ToLower().Contains("birthday"))
                {
                    ShowText = "You need to sign in to get access to birthday data";
                    SpokenText = "You need to sign in to get access to birthday data";
                    ...
                    // Send the bot sign-in card
                    reply = activity.CreateReply();

                    reply.Attachments = new List<Attachment>();
                    var card = new Attachment();
                    card.ContentType = "application/vnd.microsoft.card.oauth";
                    reply.Attachments.Add(card);
                    await context.PostAsync(reply);
                }
                else
                {
                    ShowText = "Unauthenticated: Cortana did not send authorization token";
                    SpokenText = "Unauthenticated: Cortana did not send authorization token";
